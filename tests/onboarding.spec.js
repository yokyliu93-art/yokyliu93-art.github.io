...
test('start with a bare 3D island then choose water',async({page})=>{
 await page.goto('/?view=onboarding');await expect(page.locator('canvas')).toBeVisible();await page.screenshot({path:'artifacts/empty-island.png'});await page.locator('#arrival-next').click();await expect(page.locator('h1')).toContainText('水，要怎样改变这座岛');await page.locator('[data-choice="river"]').click();await expect(page.locator('.choice-feedback')).toContainText('预览已更新');await page.screenshot({path:'artifacts/island-first-water.png'});await page.locator('[data-choice="none"]').click();await page.locator('#arrival-next').click();await expect(page.locator('h1')).toContainText('陪你');
});
