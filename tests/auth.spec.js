...
test('account creation leads to onboarding, login returns to the same island, and logout protects it',async({page})=>{
 ...
 await page.locator('#arrival-next').click();for(const choice of ['forest','none','balanced','open','none']){await page.locator(`[data-choice="${choice}"]`).click();await page.locator('#arrival-next').click();}
...
test('login and onboarding fit a mobile viewport',async({page})=>{await page.setViewportSize({width:390,height:844});await page.goto('/?view=auth');await expect(page.locator('#auth-world canvas')).toBeVisible();await expect(page.locator('.auth-card')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await page.screenshot({path:'artifacts/account-login-mobile.png'});});
