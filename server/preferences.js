...
export const preferenceOptions={animals:['rabbit','bird','cat','none'],light:['day','night','balanced'],home:['open','cottage','floating'],landscape:['forest','flowers','meadow','none']};
const customKeys=['world','water',...Object.keys(preferenceOptions)];
export function validPreferences(p){
 if(!p||typeof p!=='object'||Array.isArray(p)||!Object.keys(p).every(k=>customKeys.includes(k)||['variant','custom','growing'].includes(k)))return false;
