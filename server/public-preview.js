...
const mime={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.woff2':'font/woff2','.woff':'font/woff','.ttf':'font/ttf','.svg':'image/svg+xml','.glb':'model/gltf-binary','.json':'application/json','.png':'image/png','.ico':'image/x-icon','.mp3':'audio/mpeg'};
const server=createServer(async(req,res)=>{
 ...
 try{
  ...
  if(path!=='/'&&!path.startsWith('/assets/')&&!path.startsWith('/music/')&&!path.startsWith('/models/')&&!path.startsWith('/brand/')&&!path.startsWith('/images/')){res.statusCode=404;return res.end('Not found');}
