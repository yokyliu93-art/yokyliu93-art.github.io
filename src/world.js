...
import {cottage,homeInterior} from './home-interior.js';
...
export function createWorld(container, state, onCollect, onPlace, options = {}) {
  ...
  renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ReinhardToneMapping;renderer.toneMappingExposure=0.92;
  ...
  const skyUniform={top:{value:new THREE.Color('#b4d0c6')},bottom:{value:new THREE.Color('#e3ecdf')}};
  ...
  scene.fog=new THREE.FogExp2('#d5e1d2',.0042);
  const ambient=new THREE.HemisphereLight('#fffaf1','#5f8b82',1.55);scene.add(ambient);
  const sun=new THREE.DirectionalLight('#f9f0dc',2.05);sun.position.set(-12,22,12);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-16,right:16,top:16,bottom:-16,near:1,far:70});sun.shadow.bias=-.0006;sun.shadow.normalBias=.08;scene.add(sun);
  const fill=new THREE.DirectionalLight('#9fbfa5',1.05);fill.position.set(12,6,-10);scene.add(fill);
  const edge=new THREE.DirectionalLight('#e7dccf',.38);edge.position.set(-9,-2,10);scene.add(edge);
  const island=new THREE.Group();scene.add(island);
  const terrain=new THREE.Group();island.add(terrain);
  const decor=new THREE.Group();island.add(decor);
  const walker=islandWalk({camera,controls,canvas:renderer.domElement,island,scene});
  const interior=homeInterior(scene,walker,island);let ecology=state;
  ...
  const mats=new Map();
  function mat(color,extra={}){const key=color+JSON.stringify(extra);if(!mats.has(key))mats.set(key,new THREE.MeshStandardMaterial({color,roughness:.94,metalness:0,flatShading:true,...extra}));return mats.get(key);}
  ...
  function weather(){const night=state.time==='night',wet=state.weather==='rain';skyUniform.top.value.set(night?'#19243c':wet?'#829eaa':'#93c8d5');skyUniform.bottom.value.set(night?'#344455':wet?'#c7d6d0':'#e7f1df');scene.fog.color.set(night?'#668a89':wet?'#b9ccc7':'#dcebdc');sun.intensity=night?.35:wet?1.5:2.6;fill.intensity=night?.25:1.4;ambient.intensity=night?.38:wet?1.6:1.9;stars.material.opacity=night?.95:.15;rain.visible=wet;rainbow.visible=state.weather==='rainbow';renderer.toneMappingExposure=night?.85:1.05;}
  function expand(){terrain.scale.set(1+state.expansions*.1,1,1+state.expansions*.1);}
  weather();expand();
  addEventListener('pagehide',()=>interior.dispose());
  ...
  const previewEpoch=Date.now();const clock=new THREE.Timer();let elapsed=0,lastClockNotice=-1;const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
  function animate(){clock.update();const dt=Math.min(clock.getDelta(),.05);elapsed+=dt;if(state.daylight){const reading=worldClock(state.daylight,state.clockStartedAt??previewEpoch);const nextTime=reading.day?'sunset':'night';state.time=nextTime;const d=reading.daylight;skyUniform.top.value.set('#19243c').lerp(new THREE.Color(state.weather==='rain'?'#829eaa':'#93c8d5'),d);skyUniform.bottom.value.set('#344455').lerp(new THREE.Color('#e7f1df'),d);scene.fog.color.copy(skyUniform.bottom.value);sun.intensity=.16+d*(state.weather==='rain'?1.4:2.44);ambient.intensity=.38+d*1.52;fill.intensity=.25+d*1.15;stars.material.opacity=.98-d*.95;sun.position.set(Math.cos(reading.angle)*20,5+Math.max(0,Math.sin(reading.angle))*22,12);renderer.toneMappingExposure=.85+d*.2;if(Math.floor(elapsed)!==lastClockNotice){lastClockNotice=Math.floor(elapsed);window.dispatchEvent(new CustomEvent('island-clock',{detail:reading}));}}seasons?.update(dt);if(!reduced){clouds.forEach((g,i)=>{g.position.x+=dt*.07;if(g.position.x>75)g.position.x=-75;});foam.forEach((m,i)=>{m.position.y-=dt*(2+i%3);if(m.position.y<-6)m.position.y=.1;});collectibles.children.forEach((m,i)=>{m.rotation.y=elapsed*.6;m.position.y=1.75+Math.sin(elapsed*1.5+i)*.17;});neighbors.forEach((g,i)=>{g.position.lerp(neighborTargets[i],.01);g.position.y=Math.sin(elapsed*.3+i)*.35-1;});}
    ...
    updateModels(island,dt);interior.update(dt);if(options.ecosystemOnly)placed.forEach(g=>{if(g.userData.fauna){g.position.x=g.userData.originX+Math.sin(elapsed*.4)*.5;g.position.y=g.userData.originY+.25+Math.sin(elapsed*1.4)*.12;}});if(walker.active)walker.update(dt);else controls.update();if(atmosphere)atmosphere.render(dt,elapsed);else renderer.render(scene,camera);
  }
  renderer.setAnimationLoop(animate);
  addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.fov=walker.active?(walker.stats().mode==='first'?65:55):(innerWidth<700?58:36);if(!walker.active&&options.ecosystemOnly&&innerWidth<750)camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.17,innerWidth,innerHeight);else camera.clearViewOffset();camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);});
  return {land(){targetPos=null;targetLook=null;return walker.land();},takeoff(){if(interior.active)interior.leave();walker.leave();targetPos=homePosition.clone();targetLook=new THREE.Vector3();},walkInput:(key,on)=>walker.press(key,on),setView:mode=>walker.view(mode),setAvatar:source=>walker.loadAvatar(source),
    nearbyHouse(){if(!walker.active||interior.active)return null;for(const [id,g] of placed)if(g.userData.house&&g.position.distanceTo(walker.position)<3.4)return id;return null;},
    enterHouse(houseId){if(!walker.active)return false;const house=ecology.objects.find(o=>o.id===houseId&&o.kind==='house');const record=ecology.rooms.find(r=>r.houseId===houseId)||ecology.rooms.find(r=>!r.houseId);if(!house||!record)return false;interior.enter(record,ecology.objects);return true;},leaveHouse:()=>interior.leave(),room:()=>interior.room,nearRecord:()=>interior.nearRecord(),playRecord:()=>interior.toggle(),recordPlaying:()=>interior.playing,refreshRoom(next){ecology=next;const record=next.rooms.find(r=>r.id===interior.room?.id);if(record)interior.refresh(record,next.objects);},select,rotate,addObject,weather,expand,setEcology(next){ecology=next;state.growing=!!next.growing;state.worldTemplate=next.worldTemplate;terrain.children[1].geometry=next.growing?sculptedGround:flatGround;terrain.children[1].position.y=next.growing?0:.08;terrain.children.forEach((g,i)=>{if(i>=45&&g!==water&&g!==waterfall)g.visible=!next.growing;});terrain.visible=!['planet','ocean','desert'].includes(next.worldTemplate);program.load(next.program?.startsWith('// Generated starting world:')?worldProgram(next.worldTemplate,next.worldVariant||0):next.program);Object.assign(state,{weather:next.weather,time:next.time,music:next.music,daylight:next.daylight,clockStartedAt:next.clockStartedAt??previewEpoch});if(next.ecosystem){terrain.children[1].material.color.set(next.ecosystem.season==='winter'?'#dae3d4':({woodland:'#aac49a',wetland:'#91b49e',meadow:'#bccca0',alpine:'#b6c8bd'}[next.ecosystem.biome]||'#aac49a'));}for(const g of placed.values())decor.remove(g);placed.clear();next.objects.filter(o=>!o.roomId).forEach(o=>addObject({...o,type:o.kind}));water.visible=next.objects.some(o=>o.kind==='river');waterfall.visible=next.objects.some(o=>o.kind==='waterfall');collectibles.visible=false;weather();seasons?.refresh();},updateMatches(scores){scores.forEach((score,i)=>{const a=[-2.25,-.65,-1.7][i];const d=20+(100-score)*.7;neighborTargets[i].set(Math.cos(a)*d,-1,Math.sin(a)*d);const curve=new THREE.QuadraticBezierCurve3(new THREE.Vector3(0,-2,0),new THREE.Vector3(neighborTargets[i].x*.5,-5,neighborTargets[i].z*.5),neighborTargets[i]);neighborLines[i].geometry.dispose();neighborLines[i].geometry=new THREE.BufferGeometry().setFromPoints(curve.getPoints(40));});},removeObject(id){const g=placed.get(id);if(g){decor.remove(g);placed.delete(id);}},home(){visiting=null;targetPos=homePosition.clone();targetLook=new THREE.Vector3();},visit(i){select(null);visiting=i;const p=neighbors[i].position;targetLook=p.clone();targetPos=p.clone().add(new THREE.Vector3(11,9,14));},zoom(n){camera.position.sub(controls.target).multiplyScalar(n).clampLength(16,65).add(controls.target);},getCollectibleTargets(){return collectibles.children.map(m=>{const p=m.getWorldPosition(new THREE.Vector3()).project(camera);return {x:(p.x+1)*innerWidth/2,y:(1-p.y)*innerHeight/2};});},getStats(){return {clock:worldClock(state.daylight,state.clockStartedAt??previewEpoch),walk:walker.stats(),program:program.stats(),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,objects:placed.size,webgl:!!renderer.getContext()};}};
}
