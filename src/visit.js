...
import './visit.css';
const params=new URLSearchParams(location.search),id=params.get('island'),demo=params.get('demo');
...
if(!id&&!demo){
 ...
 try{
 if(id){const record=await api('/islands/'+encodeURIComponent(id));scene=record.scene;document.querySelector('#visit-title').textContent=record.name;document.querySelector('#visit-kind').textContent='已开放的用户岛屿 · 只读探访';}
 ...
 document.querySelector('#visit-description').textContent='先在云端看一眼，再落到这片风景里。探访不会修改岛主的世界。';
